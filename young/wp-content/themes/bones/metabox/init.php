	...
	function show() {
		...
		foreach ( $this->_meta_box['fields'] as $field ) {
			...
			switch ( $field['type'] ) {
                                case 'draggable':
                                    $string = ' 
                                        <style>.SB-droppable{border:1px solid black}</style>

                                            <script>
                                            jQuery(function() {
                                                    jQuery( ".SB-draggable" ).draggable();
                                                    jQuery( ".SB-droppable" ).droppable({
                                    ...
                                    foreach ( $field['options'] as $value => $name ) {
                                            ...
                                            $string .= "<li class='SB-draggable ui-widget-content'>$name</li>";
                                            $i++;
                                    }


                                    echo $string .= "</ul>
                                    <textarea class='SB-droppable ui-widget-header' id='SB-dock-1'>
                                    </textarea>
                                    <textarea class='SB-droppable ui-widget-header' id='SB-dock-2'>
                                    </textarea>
                                    <textarea class='SB-droppable ui-widget-header' id='SB-dock-3'>
                                    </textarea>";
                                    break;
				case 'text':
				...
				case 'multicheck':
					...
					foreach ( $field['options'] as $value => $name ) {
						...
						echo '<li><input type="checkbox" name="', $field['id'], '[]" id="', $field['id'], $i, '" value="', $value, '"', in_array( $value, $meta ) ? ' checked="checked"' : '', ' /><label for="', $field['id'], $i, '">', $name, '</label>';
						echo '<p>Paste this into the docking box:',$name,'</p></li>';
                                                $i++;
					}
					echo '</ul>';
					echo '<span class="cmb_metabox_description">', $field['desc'], '</span>';					
